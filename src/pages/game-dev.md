---
layout: ../layouts/GameDevLayout.astro
title: "Game Development Resources: Assets, Audio, Tools & References"
description: A curated guide to game-development assets, textures, audio, tools, and references for building games.
updated: "2026-09-03"
---

<nav class="resource-contents" aria-label="On this page">
  <span class="contents-label">On this page</span>
  <a href="#assets">Assets</a>
  <a href="#textures-materials">Textures &amp; materials</a>
  <a href="#audio-music">Audio &amp; music</a>
  <a href="#references-tools">References &amp; tools</a>
  <a href="#before-you-download">Before you download</a>
  <a href="#sources-attribution">Sources &amp; attribution</a>
</nav>

A working list of game-development assets, audio, references, and tools I return to while prototyping. Start broad, then narrow by discipline, art direction, and the current terms for each resource.

## Assets

Start with a pack that gets your prototype moving, then narrow by discipline and art direction. Community finds can be temporary, account-gated, or subject to licence changes.

### Start here / broad libraries

- **[itch.io free Game Assets](https://itch.io/game-assets/free)** — A broad browse for free game-development assets.
- **[OpenGameArt](https://opengameart.org/)** — Community-shared game art across formats and styles.
- **[Kenney](https://kenney.nl/assets)** — A dependable starting library for coherent prototype art.
- **[Unity Asset Store free assets](https://assetstore.unity.com/?free=true)** — Free assets collected in Unity’s marketplace.

### 2D & UI

- **[CraftPix](https://craftpix.net/freebies/)** — Free 2D game-art and sprite resources.
- **[Spriters Resource](https://www.spriters-resource.com/)** — A reference library for game sprites and visual studies.
- **[Klamtii Tree Sprites](https://klamtii.itch.io/tree-sprites)** — Tree sprites for adding readable nature to 2D scenes.
- **[Dumivid Mechanized Magic cards](https://dumivid.itch.io/mechanized-magic-2d-vector-fantasy-robot-cards-pack)** — Vector fantasy robot cards with a distinct illustrated theme.
- **[Voxybuns at-icons](https://github.com/Voxybuns/at-icons)** — An icon set, also available through the [itch.io page](https://voxybuns.itch.io/at-icons).
- **[Pixelart Japanese Food](https://pixelart.to/asset-packs/japanese-food-32)** — 32px pixel-art food for menus, shops, or collectibles.
- **[ICODOT icons](https://gleeson.itch.io/icodot)** — Compact icons for game UI and interface systems.
- **[0-mem0ry Post Apoc Office](https://0-mem0ry.itch.io/post-apoc-office-mega-bundle-free)** — A free post-apocalyptic office bundle for environmental storytelling.
- **[Ragnir 4-season trees](https://ragnir123.itch.io/4-season-trees-pack-v1)** — Trees spanning four seasons for changing environments.
- **[mi-dev steampunk sprites](https://mi-dev.itch.io/steampunksprites)** — Steampunk-themed sprites for a mechanical world.
- **[Patrick Winter Heimlich](https://patrick-winter.itch.io/heimlich)** — A focused 2D asset pack with a specific visual identity.
- **[Styloo Company Characters](https://styloo.itch.io/company)** — Character resources for a company or workplace setting.
- **[SBS Tiny Texture Pack 2](https://screamingbrainstudios.itch.io/tiny-texture-pack-2)** — Small textures useful for compact 2D scenes and surfaces.

### 3D & environment

- **[Quaternius](https://quaternius.com/)** — A broad source of approachable 3D game assets.
- **[Poly Haven](https://polyhaven.com/)** — A library for high-quality 3D assets, HDRIs, and environments.
- **[Sketchfab downloadable models](https://sketchfab.com/search?features=downloadable)** — Searchable models marked as downloadable.
- **[Mixamo](https://www.mixamo.com/)** — Character animation and rigging resources for quick iteration.
- **[Gamedev.tv Bloom & Burrow](https://gamedev.tv/assets/spring-assets)** — A stylized spring-themed environment collection.
- **[Vertex Horizons](https://vertexhorizons.gumroad.com/)** — 3D environment and asset releases.
- **[AssetPlus for Godot](https://store.godotengine.org/asset/moongdev/assetplus/)** — An asset resource for Godot projects.
- **[Enterables Fab](https://www.fab.com/sellers/Enterables)** — Enterable environment assets on Fab; see also [their free CGTrader models](https://www.cgtrader.com/3d-models?author=Enterables&free=1&per_page=120&sort_by=highest_price).
- **[Julio Sillet](https://juliosillet.gumroad.com/)** — Stylized 3D asset releases and packs.
- **[KayKit Board Game Bits](https://kaylousberg.itch.io/board-game-bits)** — Modular board-game-inspired 3D pieces.
- **[Synty Bubblegum Killstick](https://syntystore.com/products/polygon-bubblegum-killstick)** — A stylized Polygon asset with a playful action tone.
- **[RAD River Forest](https://radcreations.itch.io/river-forest)** — A river-and-forest environment kit.
- **[Leartes Cosmos weekly free assets](https://cosmos.leartesstudios.com/free)** — A regularly refreshed stream of free 3D assets.
- **[Minifantasy Towns II](https://krishna-palacio.itch.io/minifantasy-3d-towns-ii)** — Stylized fantasy town-building pieces.
- **[Sigils Vault Modular Dungeon](https://sigilsvault.itch.io/modular-dungeon-kit-v10)** — Modular dungeon construction assets.
- **[Luminous Dice tiny voxel nature](https://luminous-dice.itch.io/tiny-voxel-nature-pack)** — Tiny voxel nature props and scenery.
- **[Kenney Tiny Farm](https://kenney.nl/assets/tiny-farm)** — Compact farm-themed pieces for a small-scale world.
- **[Kenney Modular Cave](https://kenney.nl/assets/modular-cave-kit)** — Modular cave geometry for quickly assembling spaces.
- **[Mrtnli Abandoned Hospital](https://mrtnli.itch.io/free-abandoned-hospital-psx-style)** — A PSX-style abandoned hospital environment.
- **[ITHappy Mini Series](https://ithappystudios.com/mini-series/)** — Small-scale 3D environment and prop resources.
- **[Minifantasy Elven Kingdom](https://krishna-palacio.itch.io/minifantasy-3d-elven-kingdom)** — Stylized pieces for an elven fantasy kingdom.
- **[Sigils Vault Modular Cathedral](https://sigilsvault.itch.io/modular-cathedral-kit)** — Modular cathedral architecture for fantasy scenes.
- **[NASA Dragonfly](https://nasa3d.arc.nasa.gov/detail/dragonfly)** and **[Apollo 11 Command Module](https://nasa3d.arc.nasa.gov/detail/apollo11-command-module)** — NASA 3D models for science and space projects.
- **[KayKit Forest Nature](https://kaylousberg.itch.io/kaykit-forest)** — Forest nature pieces for stylized 3D scenes.

## Textures & materials

- **[ambientCG](https://ambientcg.com/)** — Material textures for building a varied surface library.
- **[CGBookcase](https://www.cgbookcase.com/)** — A browsable collection of texture materials.
- **[Poly Haven brass vase example](https://polyhaven.com/a/brass_vase_01)** — A concrete Poly Haven asset example for inspecting a material-ready model.
- **[ambientCG Wood 066](https://ambientcg.com/view?id=Wood066)** — A specific wood material to test in a scene.

## Audio & music

- **[Freesound](https://freesound.org/)** — Community sound effects and recordings for searching by sound.
- **[Sonniss GameAudioGDC archive](https://sonniss.com/gameaudiogdc/)** — Large game-audio archives for prototyping and sound design.
- **[Pixabay audio](https://pixabay.com/music/)** — A broad music library for finding tracks and ambience.
- **[We Love Indies sound packs](https://www.weloveindies.com/en/soundpacks)** — Sound packs aimed at independent game projects.
- **[Romain Raynal OpenSounds](https://www.romainraynal.fr/en/opensounds)** — Open sound resources from Romain Raynal.
- **[Crow Hill Pocket Strings](https://youtu.be/_IqrWA6X3Sw)** — A Pocket Strings music resource shared on YouTube.
- **[MassWAV2FLAC](https://github.com/toasterberry/MassWAV2FLAC)** — A utility for converting batches of WAV files to FLAC.

## Useful game-dev references and tools

- **[Alex Kullman’s game developer resources](https://docs.google.com/spreadsheets/d/1QhFyPfYSjHv7PjibGrslF3mNW_CIDXWv9o-iQgLbu1o/edit?usp=sharing)** — An extensive collection of game-development resources curated and maintained by Alex Kullman since 2019.
- **[Deepnight Game Feel](https://deepnight.net/games/game-feel/)** — Notes and examples for making game actions feel better.
- **[Polyglot Gamedev localization sheet](https://docs.google.com/spreadsheets/u/0/d/17f0dQawb-s_Fd7DHgmVvJoEGDMH_yoSd8EYigrb0zmM/htmlview)** — A practical localization reference sheet.
- **[Pixelnest PressKit](https://github.com/pixelnest/presskit.html)** — A GitHub-based press-kit template.
- **[PressKitty](https://impress.games/press-kitty)** — A tool for assembling a game press kit.
- **[Game Accessibility Guidelines](https://gameaccessibilityguidelines.com/full-list/)** — A checklist of accessibility considerations for games.
- **[Game UI Database](https://www.gameuidatabase.com/index.php)** — A visual reference for game interfaces.
- **[PadCrafter](https://padcrafter.com/index.php)** — A resource for controller and input references.
- **[Seyed’s publishers list](https://docs.google.com/spreadsheets/u/0/d/15AN1I1mB67AJkpMuUUfM5ZUALkQmrvrznnPYO5QbqD0/htmlview#gid=2049353894)** — A community-maintained publisher research sheet.
- **[Level Design reference database](https://level-design.org/referencedb/index.php?/category/29)** — Reference material for studying level design.
- **[Adobe Color](https://color.adobe.com/)** — Tools for exploring and testing color palettes.
- **[PureRef](https://www.pureref.com/)** — A focused workspace for collecting visual references.
- **[AssetHoard](https://assethoard.com/)** — A place to browse curated game-asset finds.

## Before you download

Listings and policies can change. Before downloading or shipping with a resource, open its current source page and check the licence, whether commercial use is allowed, engine and file-format compatibility, and whether the download is still available. Keep a note of the relevant terms and attribution requirements for your project.

## Sources & attribution

This page was assembled from the following leads. The Reddit thread and Discord channel are community leads, not independently verified endorsements:

- **[AssetHoard — “Where to Find Free Game Assets in 2026”](https://assethoard.com/blog/where-to-find-free-game-assets-2026)** — The source article behind many of these starting points.
- **[Reddit — “Where I actually find free game assets (compiled)”](https://www.reddit.com/r/gamedev/comments/1qhuwg8/where_i_actually_find_free_game_assets_compiled/)** — A community compilation and discovery lead.
- **[AssetHoard Discord — #awesome-assets](https://discord.com/channels/1454360198281035799/1494643699651710976)** — A community channel for ongoing finds.

This list is intentionally editable: add another `##` section when a new part of the game-making process deserves its own shelf.
